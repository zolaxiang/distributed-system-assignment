...
char *a[capacity];
int head=0, tail=0, num=0, end_program=0; //num: number of strings stored. end_program: when EOF is input.
...
int main()
{
	...
	while(1)
	{		
		if(end_program && num==0) //EOF has been input, and no unsent requests
			exit(0);
		if(num>0)
        {
			int i, len=strlen(a[head])+1;
			char bytes[4];     //transform integer to four separate bytes.
			for(i=0;i<4;i++)
			...
			int received=0;
			while (received < len) //in case we need multiple recv()s.
			{
